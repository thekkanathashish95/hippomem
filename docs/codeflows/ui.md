...
- **EventDetailPanel** (when event selected or loading detail): Shows core intent, event kind badge (summary / episode / entity), active/dormant status, relevance score bar, then:
  - **Summary** box (blue tint) — `summary_text` if present (post-consolidation).
  - **Updates** — consolidated updates (`updates` field), white text, blue-grey left border.
  - **Pending** — pending updates since last consolidation (`pending_facts`), amber label, muted text, amber left border.
  - Timestamps (created, last updated) and **Connected Memories** (neighbor edges with weight). Close clears selection.
...
- **Header**: “Self Memory” + subtitle “What hippomem has learned about you” + `RefreshButton` (re-fetches traits on demand).
...
- **Data fetch**: `useCallback`-wrapped `load()` called on mount and on refresh; only re-fetches when `userId` changes or user clicks refresh.
...
- **Header**: “Entities” + subtitle “People, places, and things hippomem has learned about” + `RefreshButton` (re-fetches on demand).
- **Content**: Entities grouped by type (People, Pets, Organizations, Places, Projects, Tools, Other). Each **EntityCard**:
  - Canonical name + reinforcement count badge.
  - `summary_text` (if present — generated by consolidation).
  - **Consolidated facts** (`facts`) — rendered by `FactList`; white text, blue-grey left border; first 3 shown, “+N more” expand.
  - **Pending facts** (`pending_facts`) — rendered by `FactList` with `pending=true`; amber “Pending” label, muted text, amber left border; visible immediately after encode, before consolidation.
  - “No facts yet.” fallback when both lists are empty.
  - First/last observed dates.
- **Data fetch**: Same `useCallback`-wrapped `load()` pattern as SelfMemoryView.
